...
public final class GLUProjection {
    ...
    public static float[] convertBounds(Vec3d pos, float partialTicks, int width, int height) {
        ...
        Projection projection = GLUProjection.getInstance().project(pos.x - Mod.mc.getRenderManager().viewerPosX, pos.y - Mod.mc.getRenderManager().viewerPosY, pos.z - Mod.mc.getRenderManager().viewerPosZ, ClampMode.NONE, true);
        if (projection == null) {
            return null;
        }
